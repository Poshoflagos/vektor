...
import { calculatePathScores, getTop3Paths, getMainRecommendation, getRecommendedBundle } from "../logic/scoring"
...
function PathResults({ userProfile, pathScores, setPathScores, recommendedPaths, setRecommendedPaths, setCurrentScreen, setSelectedPath }) {
  ...
  useEffect(() => {
    if (userProfile && !pathScores) {
      const scores = calculatePathScores(userProfile)
      const top3 = getTop3Paths(scores)
  ...
  const recommendation = getMainRecommendation(userProfile, pathScores)
  const bundle = getRecommendedBundle(userProfile)

  const top3Ids = [recommendedPaths.best?.[0], recommendedPaths.alternative?.[0], recommendedPaths.stretch?.[0]]
  const labels = ["⭐ Best Match", "💪 Strong Alternative", "🚀 Stretch Goal"]

  const paths = top3Ids.map(id => AI_WEB3_OPPORTUNITIES.find(p => p.pathId === id)).filter(Boolean)

  return (
    <div style={styles.container}>
      <div style={styles.inner}>
        <h1 style={styles.title}>Your Path Results</h1>
        <p style={styles.subtitle}>Based on your profile, here are your top 3 matches</p>

        <div style={styles.recommendation}>
          We recommend you start with: <strong style={{ color: "#00ff88" }}>{recommendation.toUpperCase()}</strong>
        </div>

        {bundle?.fastestMoneyPath && (
          <div style={styles.fastMoney}>
            ⚡ Fastest money path for you: <strong style={{ color: "#ffcc00" }}>{bundle.fastestMoneyPath.name}</strong>
          </div>
        )}
        ...
        <div style={styles.grid}>
          {paths.map((path, i) => (
            <div key={path.pathId} style={styles.card}>
              ...
              <h2 style={styles.pathName}>{path.name}</h2>
              <div style={styles.scoreBar}>
                <div style={{ ...styles.scoreFill, width: `${(pathScores[path.pathId] || 0)}%` }} />
              </div>
              <p style={styles.scoreText}>{pathScores[path.pathId] || 0}% match</p>
              <p style={styles.difficulty}>Difficulty: {path.difficulty}</p>
              <p style={styles.meta}>⏱ {path.timeline}</p>
              <p style={styles.meta}>💰 {path.firstMoneyRoute}</p>
              ...
              <p style={styles.metaGreen}>✅ {path.whyItFits}</p>
              <p style={styles.metaRed}>⚠️ {path.whyItMayNot}</p>
              {path.beginnerWarning && (
                <p style={styles.warning}>📌 {path.beginnerWarning}</p>
              )}
              <button
        ...
        <button
          style={styles.btn}
          onClick={() => setCurrentScreen("guideSelect")}
        >
...
const styles = {
  ...
  recommendation: { background: "#111", border: "1px solid #00ff88", borderRadius: "8px", padding: "16px 24px", color: "white", marginBottom: "12px", fontSize: "14px" },
  fastMoney: { background: "#111", border: "1px solid #ffcc00", borderRadius: "8px", padding: "12px 24px", color: "#aaa", marginBottom: "24px", fontSize: "13px" },
